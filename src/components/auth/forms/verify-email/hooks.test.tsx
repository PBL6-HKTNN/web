...
describe('useVerifyEmailForm', () => {
  ...
  it('updateField updates formData', () => {
    ...
    vi.spyOn(authHook, 'useVerifyEmail').mockReturnValue(
      createMutationMock()
    )

    const { result } = renderHook(
      () => useVerifyEmailForm('test@gmail.com', '', undefined),
      { wrapper }
    )
  ...
  it('handleSubmit calls mutation and navigates when valid', async () => {
    ...
    const { result } = renderHook(
      () => useVerifyEmailForm('test@gmail.com', '123456', undefined),
      { wrapper }
    )
  ...
  it('handleSubmit shows error toast when API fails', async () => {
    const mockErrorObj = new Error('Verification failed. Please check your token and try again.')
    ...
    expect(mockError).toHaveBeenCalledWith('Verification failed. Please check your token and try again.')
